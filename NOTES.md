...
- With that fixed, **the build step passed — all the previously-uncompiled Phase 0–2 Swift compiled clean on the first real attempt.** The test step then failed with `Could not find test host for PatchTrackerTests`, right after package resolution with no build output at all — first suspected `CODE_SIGNING_ALLOWED=NO` (dropped it from the test invocation), but the identical error persisted, ruling that out. Root cause: `xcodebuild test`'s combined build+test action is a known-flaky sequencing in fresh CI environments — it doesn't reliably build the hosted test bundle's host app before checking for/installing it. Fix: split into `xcodebuild build-for-testing` then `xcodebuild test-without-building`, both pinned to an explicit shared `-derivedDataPath DerivedData` so there's no ambiguity about where the host app landed between the two invocations.
