...
- With that fixed, **the build step passed — all the previously-uncompiled Phase 0–2 Swift compiled clean on the first real attempt.** The test step then failed with `Could not find test host for PatchTrackerTests`, right after package resolution with no build output at all. Two wrong guesses before the real cause: (1) `CODE_SIGNING_ALLOWED=NO` — dropped it from the test invocation, identical error persisted; (2) `xcodebuild test`'s combined build+test action being flaky about build ordering — split into `build-for-testing` + `test-without-building` with a shared `-derivedDataPath`, and it failed identically **even on `build-for-testing` itself**, before attempting to build anything — which ruled out sequencing entirely. **Real root cause:** the `project.yml` shorthand `scheme: testTargets: [PatchTrackerTests]` on the app target doesn't mark the app target itself `buildForTesting` in the generated scheme's Build action — so `PatchTracker.app` was never scheduled to be built as part of a test run at all, regardless of command form. Fix: replaced the shorthand with an explicit top-level `schemes:` block in `project.yml` (`build.targets: { PatchTracker: [run, test, profile, analyze, archive], PatchTrackerTests: [test] }`) so the app target is explicitly `buildForTesting`.
